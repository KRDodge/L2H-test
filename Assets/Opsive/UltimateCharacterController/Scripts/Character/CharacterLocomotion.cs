            ...
            for (int i = 0; i < hitCount; ++i) {
                ...
                if (closestRaycastHit.distance == 0) {
                    ...
                    if (!overlap) {
                        ...
                        if (SingleCast(activeCollider, m_GravityDirection * (m_MaxStepHeight + verticalOffset + c_ColliderSpacing), 
                                            Vector3.ProjectOnPlane(m_MoveDirection, -m_GravityDirection) + m_Transform.up * (m_MaxStepHeight + verticalOffset + c_ColliderSpacing))) {
